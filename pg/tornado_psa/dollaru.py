...
class SYZHandler(tornado.web.RequestHandler):
    def get(self):
        salida=[]
	for tre in  csv.reader(open("csv/tree.csv")):
		salida.append([tre[0],tre[1],tre[2],"green"])
	self.render("syzame.html", title="My title",datos=sorted(salida, key=lambda sali: sali[1]))
