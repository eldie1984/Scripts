...
#====================================================================================================================
#ARCHIVO: informes_parametricos.sh
#====================================================================================================================
#VERSION    FECHA       MODIFICO       NOTAS
#====================================================================================================================
#  1        06/06/05       LED          -Version Inicial
...
variables(){
	...
	APP_NAME=informes_parametricos         # Nombre del Aplicativo. Todos los archivos de log y out llevaran este nombre.(PARAMETRO)
	APP_FUNCION="Corre los reportes parametrizados"
	
	#DESCOMENTAR EL QUE CORRESPONDA Y EN EL CASO DE SE SER UN JOB REEMPLAZAR EL NOMBRE POR EL CORRECTO!!!!!!
	#LANZADOR="JOB de Mainframe: COLOCAR NOMBRE DEL JOB"  #Si el script es lanzado por un job de mainframe.
	LANZADOR="Cron de Unix"                              #Si el script es lanzado por el Unix.
...
directorios(){
	...
	DIR_JARS=${DIR_APP}/jars 		       # Directorio de archivos jar y class 
...
TestConexionOracle(){
	...
	cat ${FILE_AUX} | grep "ORA-01017" 2>/dev/null
 	if [ $? = 0 ]; then
		...
		echo "Error de conexion a la BD" >> ${FILE_CEL}
		return 1	
	else
...
EnviarEmail () {
   ...
   echo "SERVIDOR UNIX: $HOSTNAME " >> ${FILE_MAILBODY}
...
FILE_SP_OUT=${DIR_OUT}/${APP_NAME}.${TIMESTAMP}.SQL.`echo $1 | awk -F. '{printf "%s.out",$1}'`
   ...
   if [ !  -f ${FILE_SP} ]; then
  	...
  	sqlplus ${USUARIO_H_BD} @${FILE_SP} $2 $3 $4 $5 > ${FILE_SP_OUT}	
...
CargarArchivo(){
      ...
      sqlldr userid=${USUARIO_H_BD}, control=${FILE_LOAD_CTL}, log=${FILE_LOAD_OUT}, bad=${FILE_LOAD_BAD}, data=${FILE_LOAD_DATA}, discard=${FILE_LOAD_DIS}, errors=1000000, silent=all, bindsize=2470000, rows=100
...
# 1° classpath
# 2° clase main
# 3 ... 6: parametros varios

EjecutarJava ${DIR_JARS}/classes.jar:${DIR_JARS}/clarin_mail.jar:${DIR_JARS}/mailer.jar:${DIR_JAVA}/informes.jar clarin.informesparametricos.Informar ${DIR_JAVA}
RET=$?
if [ ${RET} != "0" ]; then
	echo
	resumen ABORT
